...
type Uploader interface {
	UploadMetrics(ctx context.Context, tenantID string, batch metrics.Batch) error
	UploadSnapshot(ctx context.Context, tenantID string, snap metrics.Snapshot) error
}
...
func (d discardUploader) UploadMetrics(_ context.Context, tenantID string, batch metrics.Batch) error {
	d.logger.Warn("采集数据无上报出口，已丢弃（未配置 CONSOLE_URL？）",
		"tenant_id", tenantID, "datasource_id", batch.DatasourceID,
		"metrics", len(batch.Metrics))
	return nil
}

func (d discardUploader) UploadSnapshot(_ context.Context, tenantID string, snap metrics.Snapshot) error {
	d.logger.Warn("快照无上报出口，已丢弃（未配置 CONSOLE_URL？）",
		"tenant_id", tenantID, "datasource_id", snap.DatasourceID, "kind", snap.Kind)
	return nil
}
