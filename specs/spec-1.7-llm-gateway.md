...
| 2026-08-17 | **spec-1.8 金丝雀回填两条**：① **Q3 ★A（agent 统一说 Responses、由 LiteLLM 桥接）在真客户端 + Kimi 的工具回合上翻车**——codexgo 第二次采样带回 `function_call`+`function_call_output`，桥接成 chat 时 Moonshot 报 "an assistant message with 'tool_calls' must be followed by tool messages responding to each 'tool_call_id' … update_plan:0"（Kimi 的 tool_call id 形如 `name:index`，桥接后 tool 消息的 id 对不上）；按本 spec R2 预案退到 **Q3 备选 B**：agent-runtime 缺省线协议改 **chat**（`AIRUSH_AGENT_LLM_WIRE_API=chat|responses`，codexgo `NewChatModelClient`，LiteLLM 原生转发不改写 id），Responses 保留为可选。chat 线下真 Kimi 一轮 + 一次 update_plan 工具回合通过（spec-1.8 §11）。② **§11 2026-08-15 ③ 登记项（Meter 4xx 转 error）复核**：codexgo 客户端把它当"网络错误"重试 4 次才报错，且 turn 的 error 事件里只有 "network error"——agent-runtime 侧在 Meter 外包一层把 AR_QUOTA_EXCEEDED→429、网关 4xx→同状态码的合成响应（正文只带平台错误码，上游正文仍不透传），客户端/采样层对 4xx 不重试；5xx/超时仍走原路（应重试）。Meter 本身不动 |
