...
| 日期 | 修订 | 依据 |
|---|---|---|
| 2026-08-10 | **Go 覆盖率口径修订（user approve）**：模块覆盖率 = 单测 profile ∪ 集成 profile（`bin/cover/integration/`）按语句块去重合并，阈值 80% 不变；§10 中"口径不含集成用例"对 Go 后端不再适用（Python/前端口径不变）。理由：首个 DB 重模块（spec-1.1 console）的 SQL 粘合与装配层正确性证据只能由集成测试给出，fake-DB 单测是无证明力的凑数；合并口径下"未被任何测试执行的代码"才是真缺口。阻断点收敛至 ci/integration（唯一拥有完整 profile 的 job），ci/test 保持报告态。 | spec-1.1 实施中发现，user 会话决策（选项 1） |
