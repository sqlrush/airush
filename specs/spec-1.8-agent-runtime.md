...
> **shipped** — user approve 2026-08-16，实施完成 + dev-verify ALL PASS，PR #32 合并 2026-08-17；Kimi K3 金丝雀 PASS（chat 线，见 §11）。（§8 Q1-Q9 **全采 ★**；`agents.default_model` 追加迁移一并批准；codexgo 传递依赖清单经批（uuid + go-toml/v2 + klauspost/compress + uniseg）；CI 二次 checkout 走公开仓无需凭据；实施中 D0.10–D0.14 五处 codexgo 补丁、抽核包 `pkg/` 布局、LLM 线协议缺省 chat（spec-1.7 Q3 备选 B）由 user 2026-08-17 裁决接受。）
...
- [x] Meter 4xx→error 契约在真客户端下验证或修正（T18）：4xx 曾被当网络错误重试 4 次；runtime 侧合成同码响应（不重试），5xx/超时仍重试；
- [x] Kimi K3 金丝雀：一轮真对话 + 一次工具调用成功（chat 线；Responses 桥接在工具回合失败 → spec-1.7 Q3 备选 B）；Mac 本地，key 不进仓；
...
| 2026-08-17 | **合并 PR #32（main 81e5cd3）；user 裁决**：Kimi key 不轮换、授权我用现有 key 做金丝雀；其余全部按推荐（D0.10–13 接受、pkg/ 维持、审批门 fail-closed、会话每轮领取/释放、runtime 自有审批事件、agent 指令并入 BaseInstructions、Extended 持久化、无跨租户旁路、内部 API 多两条端点；遗留 T18/T22/T23 单独小 PR；Go 补丁版另开 chore PR） |
| 2026-08-17 | **Kimi K3 金丝雀 PASS**（`deploy/scripts/mac-agent-canary.sh`：console → agent-runtime → LiteLLM（kind，`mac-llm-real-provider.sh` 装的 chat-kimi）→ 真 K3；要求模型先调 codexgo 内置 `update_plan`——无 MCP 也存在的真实 function_call 回合）：事件流 session_meta/turn_context/task_started/response_item×4（function_call + function_call_output 各 1）/token_count×2/agent_message/task_complete，无 error 事件；助手中文回复非 mock；Meter 记账 used_this_month 2257→3728；runtime/LiteLLM 日志 0 次 key 片段。**过程中修掉三处**：① 首跑 Kimi 看到了 `view_image` 与托管 `web_search`——codexgo D0.14 `view_image` 按模型 `input_modalities` 广告（spec_plan 语义），airush 声明纯文本模型 + `SetProviderCapabilitiesResolver`（无托管搜索/图片生成）+ `WebSearchMode=disabled`（用例 `TestAdvertisedToolsArePlatformShaped`）；② **Responses 桥接在工具回合丢 tool_call_id → 退 spec-1.7 Q3 备选 B**：缺省线协议 chat（`AIRUSH_AGENT_LLM_WIRE_API`），Responses 可选；chat 线用 testkit/mockllm 加集成用例（工具回合 + usage）；③ **T18 收口**：Meter 4xx 映射的错误在客户端被当网络错误重试 4 次——`quotaAwareTransport` 扩成"网关 4xx/配额 → 同码合成响应（不重试）、5xx/超时透传（重试）"，spec-1.7 §11 同步登记。codexgo.lock → febc404 |
