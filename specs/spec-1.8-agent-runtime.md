...
| 2026-08-17 | **更正 2026-08-16 Q7 行的一处结论**："其它模块的构建不受 agent-runtime 的 replace 影响" 只对孤立最小工作区成立；真实工作区里 libs/obs 等模块的依赖版本要经 MVS 汇总全部 workspace 模块（含 agent-runtime → codexgo）才能解析，`../codexgo` 缺失时 console/gateway 也编不过。故 CI 所有 go job 与 `deploy/docker/go.Dockerfile` **无条件**先按 lock 取 codexgo（实际实现本就如此，只是文字结论错了）。另：`go.work` 加 `toolchain go1.26.6`——main 的 security 定时 job（govulncheck）报 GO-2026-6218（net/url）/GO-2026-6091（html/template），修复版 1.26.6；toolchain 指令是唯一钉点，setup-go/`golang:1.26` 浮动到该补丁版之下时也会自动取 |
