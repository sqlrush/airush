...
| 环境 | 形态 |
|---|---|
| **本地部署（当前主形态，2026-08-10 user 定）** | 全套进 k8s：Helm 内置存储（`storage.builtin=true`），生产级数据保护按 §1.2 执行（operator 路线随 spec-4.5 加固）——与开发/测试同路径，交付即主线 |
| 开发/测试 | 全套进 k8s（kind + Helm 内置存储，`make dev-up` 一键全栈） |
| 云 SaaS（后续演进选项） | **存储不进业务 k8s 集群**：PG/Redis 用云托管（RDS 等），对象存储托管；Neo4j 独立部署（无成熟国内托管） |

云 SaaS 形态存储不进 k8s 的理由（届时适用）：小团队把主备/备份/监控外包给云厂商是
决定性减负；计算与存储故障域分离；数据库 IO 绕开 CSI 层损耗。本地部署主形态下
§1.2 的数据保护清单从"Stage 4 必含"提前为**随存储 subchart 落地即适用**。
...
| 2026-08-10 | §1.1 形态调整（user 定）：本地部署 k8s 升为当前主形态（Helm 内置存储为主线），云 SaaS 托管降为后续选项；§1.2 数据保护适用时点相应提前 |
