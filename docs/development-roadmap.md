...
| spec-1.8 | **frozen · 实施完成待 review/合并**（codexgo `airush-core` D0.1–D0.13 全部落地（含 D0.9 缝合与 `pkg/` 布局；spec 50 changelog / DEVIATIONS 登记）；airush：0006 四表 RLS + pgstore（contracttest/agentgraphtest 全过）+ `AgentCore` 运行时（无状态领取/释放、外置队列、sweeper、SSE PG tail、租户贯穿、审批门、排水/恢复）+ 内部 API + console 反代 + Helm/CI/Dockerfile 接 codexgo lock；集成用例真 PG + 进程内 Responses 假供应商全过，Mac lint 零告警、合并覆盖率 82.7%；dev-verify 接 agent-runtime 段。dev-verify ALL PASS。**未做**：T18 Meter 4xx 复核、Kimi 金丝雀）| 2026-08-16 起草并 approve；2026-08-17 实施完成 |
