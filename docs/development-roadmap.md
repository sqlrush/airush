...
| spec-1.8 | **shipped**（PR #32 合并 2026-08-17：codexgo `airush-core` D0.1–D0.14 + `pkg/` 布局；airush 0006 四表 RLS + pgstore（contracttest/agentgraphtest）+ `AgentCore` 运行时（无状态领取/释放、外置队列、sweeper、SSE PG tail、租户贯穿、审批门、排水/恢复）+ 内部 API + console 反代 + Helm/CI/Dockerfile 接 codexgo lock；集成用例真 PG + Responses 假供应商 + mockllm chat 全过，lint 零告警、合并覆盖率 82.7%；dev-verify ALL PASS；**Kimi K3 金丝雀 PASS**（一轮真对话 + update_plan 工具回合；Responses 桥接在工具回合丢 tool_call_id → 缺省线协议改 chat，spec-1.7 Q3 备选 B）；实测修正：view_image/web_search 不该广告、启动维护放后台、SSE 经 obs 中间件需 Unwrap。遗留 T22/T23 dev 形态）| 2026-08-16 起草并 approve；2026-08-17 实施完成、合并 |
