...
| spec-1.5 | **frozen · 实施完成**（T1-T22 全过；AD-10 等效隔离四项门槛 T7-T10 全绿——其中"越权写被拒"初验时没拦住，补 `check_option` 才堵上；R1 基准经视图写入退化 **5.9%**（门槛 30%）故 §8 Q2 选项 A 落地；表数收敛承诺兑现：采集侧固定 3 张表，加采集能力/加引擎只加编译期目录常量；实施中修掉采集器租户上下文漏传（内存 Sink 掩盖了它）与 kind 冷启动两处工程缺陷；覆盖率 console 83.4%/libs-metrics 86.3%/connector 87.3%/gateway 81.0%；dev-verify ALL PASS 含 Direct 与 Connector 两通道落库；code review 后 Top N 排名端点显式 501 移交 spec-1.11、上报链补数据源归属校验）| 2026-08-14 起草并 approve；08-15 实施完成 + review 收口 |
