...
| 2026-08-16 | **策略落地形态调整（user 定）**：策略 C 的"先进 codexgo 主线再抽核"改为——余下对齐工作全部并入 spec-1.8 D0，在 codexgo 抽核分支 `airush-core` 上按 codexgo 纪律实施（DEVIATIONS 登记 forward-synced 项），是否合回主线由 codexgo 纪律另定。簇 B 只取"失败上抛 + 并发上限"，簇 C 在 airush 侧做（1.7 已覆盖租户级），簇 D 取接口与语义、不取本地文件工程，簇 E 的核心（上下文窗口/压缩 item）随 core 盘点 ② 一并进 1.8 |
