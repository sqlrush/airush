...
| AD-2 | SaaS 连接客户数据库 | **客户侧 Connector 反向隧道**（outbound-only mTLS 长连接） | 公网白名单直连否决：企业客户不接受数据库公网暴露 |
...
| AD-4 | 凭据边界 | **数据库凭据只存客户侧 Connector**（本地加密），平台永不持有 | 平台集中保管否决：单点爆炸半径覆盖全部客户数据库 |
...
- **Stage 1** 单租户端到端 MVP：MySQL 协议族全链路（Connector→采集→agent→skill→控制台）；
- **Stage 2** 多租户与安全：RLS、RBAC、审批流、凭据与证书体系；
- **Stage 3** 引擎扩展与知识/记忆：PG 协议族、达梦、Graphiti 记忆库、合规检查包；
- **Stage 4** 生产化与商业化：计费、报表、高可用、私有化打包（顺带能力）。
