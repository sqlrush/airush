...
1. **凭据边界（AD-4，分模式，2026-08-10 修订）**：Connector 模式凭据只存客户侧；
   直连模式（本地部署）凭据仅存平台 secret 管理（加密存储，禁明文入库）；
   两种模式下，代码、镜像、日志、LLM prompt 中出现**明文**数据库凭据 = P0 事故；
2. **租户边界（AD-10，2026-08-14 修订）**：控制面所有租户数据表必须带 tenant_id，
   且隔离**由数据库强制**——默认 RLS；仅当 RLS 与存储引擎特性硬冲突（如 TimescaleDB
   列存压缩）才可用等效形态（基表零授权 + security_barrier 视图 + WITH CHECK OPTION），
   须在 spec 逐表登记理由 + 四项集成用例；**禁止仅靠应用层 WHERE 过滤实现隔离**；
   跨租户语义的任何变更是硬门槛（见规则 5）；
